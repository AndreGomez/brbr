...
class ServiceReview extends Component {
	...
	getData = async () => {
		...
		try {
			...
			const paymentMethod = `...${filter[0].card_number.slice(filter[0].card_number.length - 4)}`
			this.setState({
				paymentMethod
			})
		} catch (error) {
					...
					loading ?
						...
						>
							...
							<Text
								style={styles.price}
							>
								{currentUser.country === 'mx' ? `MNX $${parseFloat(price).toFixed(2)}` : `USD $${parseFloat(price).toFixed(2)}`}
							</Text>
							{
